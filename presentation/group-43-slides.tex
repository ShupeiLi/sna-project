...
\begin{frame}
   ...
   \end{columns}
\end{frame}
\note{We apply node2vec algorithm on an open source dataset AIFB. Results of visualization are showed on the slide. Each node in AIFB belongs to a class. The total number of classes is four. We set the number of epochs to 100. It is easy to see that nodes embeddings are changed from chaos into order during the training of node2vec.}
